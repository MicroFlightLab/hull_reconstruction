function SparseMovieLoader_4cam_w_preds(preds3d_smooth_ew_frame,allCams,preds_start_ind)
%{
Description:
...
%}
    %% create all controls
    %% create axes
    handles.preds_handles=[];
    handles.preds_start_ind=preds_start_ind;
    handles.allCams=allCams;
    handles.preds3d_smooth_ew_frame=preds3d_smooth_ew_frame;
    ...
    function LoadButtonCallback(~, ~)
       % [fileNames,pathName] = uigetfile('*.mat','Select the sparse file/s',...
        ...
        if length(fileNames)>4
            %%add - error
            warndlg('You can load up to 4 movies at a time!')
            return
        end
        ...
        for fileInd=1:length(fileNames)
            curr_md=handles.sparseMatFiles{fileInd}.metaData;
            handles.metaData.frameSize(fileInd,:)=curr_md.frameSize;
    ...
    function ContinuousSlide(~, ~)
        ...
        for fileInd=1:length(handles.sparseMatFiles)
            if handles.bgChkbx.Value
                ...
                newIm(sub2ind(handles.metaData.frameSize(fileInd,:),...
                ...
                newIm=zeros(handles.metaData.frameSize(fileInd,:),'like',handles.bgs{1});
                newIm(sub2ind(handles.metaData.frameSize(fileInd,:),...
                    handles.sparseMovies(fileInd,round(handles.frameSlider.Value)).indIm(:,1),...
                    handles.sparseMovies(fileInd,round(handles.frameSlider.Value)).indIm(:,2)))=...
                    handles.sparseMovies(fileInd,round(handles.frameSlider.Value)).indIm(:,3);
            ...
            preds_color_mat=hot(size(handles.preds3d_smooth_ew_frame,1));
            preds_frame_ind=round(handles.frameSlider.Value)-handles.preds_start_ind+1;
            if preds_frame_ind>0
                % preds
                for node_ind=size(handles.preds3d_smooth_ew_frame,1):-1:1
                    xys(node_ind,:) = HullReconstruction.Functions.dlt_inverse(...
                        handles.allCams.cams_array(fileInd).dlt,handles.preds3d_smooth_ew_frame(node_ind,:,preds_frame_ind));
                    handles.preds_handles(end+1)=...
                        plot(handles.hAxes(fileInd),xys(node_ind,1),801-xys(node_ind,2),'.','MarkerSize',10,'Color',preds_color_mat(node_ind,:));
                end
                handles.preds_handles(end+1)=...
                        plot(handles.hAxes(fileInd),xys(:,1),801-xys(:,2),'-w');
            end 
        end
